...
The *PassThru* parameter directs PowerShell to pass an object that represents the new alias through the pipeline to the Format-List cmdlet.
...
After the command completes, to open Notepad from the PowerShell command line, just type `np`.
